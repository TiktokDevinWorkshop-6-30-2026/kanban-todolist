...
function refreshDevinConfig() {
    fetch(DEVIN_API_BASE + '/config')
        .then(function(res) {
            if (!res.ok) return;
            return res.json();
        })
...
function pollDevinSessions() {
    var tracked = state.tasks.filter(function(t) { return t.devinSessionId && t.column !== 'done'; });
    if (tracked.length === 0) return;

    var changed = false;
    var promises = tracked.map(function(task) {
        return fetch(DEVIN_API_BASE + '/sessions/' + encodeURIComponent(task.devinSessionId))
            .then(function(res) {
                if (!res.ok) return;
                return res.json();
            })
